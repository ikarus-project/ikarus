...
void registerNonLinearOperator(pybind11::handle scope, pybind11::class_<NLO, options...> cls,
                               std::array<T, sizeOfIndices> baseIndices);
...
namespace Impl {
  ...
  auto indexSeqOfFunctionIndices =
      Dune::makeArrayFromIndexSequence(std::make_index_sequence<std::tuple_size_v<typename NLO::FunctionTuple>>{});
  ...
  auto maybeRegisterNonlinearOperator(pybind11::handle scope, std::array<T, sizeOfIndices> baseIndices,
                                      std::array<T, sizeOfSubIndices> subIndices) {
    // std::cout<<"====================="<<std::endl;
    ...
    /* The names of the nonlinear operators are generated by concatenating the names of the subfunctions and the indices
of the subfunctions. This is needed to not artificially insert the same nonlinear operator multiple times with different
names (inefficient) and even worse not try to insert different nonlinear operators with the same name (Python registry
import error). We do basically here NonlinearOperator_012 -> NonlinearOperator_012 NonlinearOperator_012_0 ->
NonlinearOperator_0 NonlinearOperator_012_01 -> NonlinearOperator_01 NonlinearOperator_012_01_0 -> NonlinearOperator_0
...
NonlinearOperator_012_2  -> NonlinearOperator_2
    ...
    auto [clsNonLinearOperator, notRegisteredNonLinOp] = Dune::Python::insertClass<NLO>(
        scope, name, pybind11::dynamic_attr(), Dune::Python::GenerateTypeName(Dune::className<NLO>()), includes);
...
void registerNonLinearOperatorFactory(pybind11::handle scope, pybind11::class_<NLOFW, options...> cls) {
  ...
  auto registerNonLinearOperatorL = [&scope]<size_t... i>(std::index_sequence<i...> iSeq) {
    Impl::maybeRegisterNonlinearOperator<decltype(NonLinearOperatorFactory::op(std::declval<Assembler&>(), iSeq))>(
        scope, Dune::makeArrayFromIndexSequence(iSeq));
  };
  ...
  //     [](NLOFW& self,
  //        std::optional<
  //            std::reference_wrapper<typename
  //            traits::remove_pointer_t<std::remove_cvref_t<Assembler>>::FERequirement>> req,
