...
void registerNonLinearElastic(pybind11::handle scope, pybind11::class_<NonLinearElastic, options...> cls) {
  ...
  cls.def(pybind11::init(
              [](const GlobalBasis& basis, const Element& element, const Material& mat, const LoadFunction volumeLoad) {
                return new NonLinearElastic(basis, element, mat, volumeLoad);
              }),
          pybind11::keep_alive<1, 2>(), pybind11::keep_alive<1, 3>());
  ...
  cls.def(
      ...
      [](NonLinearElastic& self, const FERequirements& req, const Dune::FieldVector<double, Traits::mydim>& local,
        ...
        if (resType == "PK2Stress")
          return self.template calculateAt<ResultType::PK2Stress>(req, local).asVec();
        else
