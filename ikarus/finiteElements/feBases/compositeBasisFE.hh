...
namespace Ikarus {
  ...
  /**
   * compositeBasisFE.hh includes a class which takes in a composite basis and it assumes
   * the local indices in a FlatInterLeaved
   * Multi-DOFs elements can be inherited from this class which uses a composite basis
   */
  template <typename Basis>
  class CompositeBasisFE {
  public:
    using RootBasis   = Basis;
    using LocalView   = typename Basis::LocalView;
    using GlobalIndex = typename LocalView::MultiIndex;
    explicit CompositeBasisFE(const Basis& p_basis, const typename LocalView::Element& element)
        : localView_{p_basis.localView()} {
      static_assert(Ikarus::Concepts::CompositeBasis<RootBasis>,
                    "You didn't pass a localview of a composite basis to this method");
    ...
    /** \brief Number of children in the compositeBasis */
