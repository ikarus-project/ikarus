...
namespace Ikarus {
  ...
  public:
    ...
    KirchhoffLoveShell(const Basis& globalBasis, const typename LocalView::Element& element, const Material& p_mat,
  ...
  protected:
    ...
    auto calculateScalarImpl(const FERequirementType& par, const std::optional<const Eigen::VectorX<ScalarType>>& dx
      ...
      for (const auto& [gpIndex, gp] : uFunction.viewOverIntegrationPoints()) {
        ...
        const auto [X,J,H] = geo.zeroFirstAndSecondDerivativeOfPosition(gp.position());
//        auto J = toEigen(geo.jacobianTranspose(gp.position()));
        const auto A1 = J.row(0);
        const auto A2 = J.row(1);
        const Eigen::Vector3<ScalarType> A3 = (A1.cross(A2)).normalized();
        const Eigen::Matrix<ScalarType,2,2> B = geo.secondFundamentalForm(gp.position());
        const Eigen::Matrix<ScalarType,2,2> A = J* J.transpose();
        const Eigen::Matrix<ScalarType,2,3> gradu = toEigen(uFunction.evaluateDerivative(gpIndex, wrt(spatialAll,Dune::on(referenceElement))));
        const auto ud1 = gradu.row(0);
        const auto ud2 = gradu.row(1);
        const Eigen::Vector3<ScalarType> a1 = A1+ud1;
        const Eigen::Vector3<ScalarType> a2 = A2+ud2;
        const Eigen::Vector3<ScalarType> a3 = (a1.cross(a2)).normalized();
