...
template <typename Basis_, bool useEigenRef_ = false>
  ...
  public:
    using Basis = Basis_;
    static constexpr int centerLineDim           = 3;
    static constexpr int quaternionDim          = 4;
    ...
    using CenterLinePositionsVector                      = Dune::BlockVector<Dune::RealTuple<double, centerLineDim>>;
    ...
    template <typename VolumeLoad = LoadDefault>
    SimoReissnerBeam(const Basis& globalBasis,
                                      const typename LocalViewBlocked::Element& element, BeamMaterial& p_material,
                                      const VolumeLoad& p_volumeLoad)
        : localView_{globalBasis.localView()},
          localViewReduced{globalBasis.untouched().localView()},
          ...
          material{p_material}
    {
      ...
      localBasisCenterLine
          = Dune::CachedLocalBasis(localView_.tree().child(Dune::Indices::_0, 0).finiteElement().localBasis());
      ...
      localBasisQuaternions
          = Dune::CachedLocalBasis(localView_.tree().child(Dune::Indices::_1, 0).finiteElement().localBasis());
...
//    }
  ...
  private:
    ...
    ScalarType calculateScalarImpl(const FERequirementType& par, Eigen::VectorX<ScalarType>& dx_) const {
      ...
      const auto geo = localView_.element().geometry();
      auto geoS= std::make_shared<decltype(geo)>(geo);
      const auto& rule = Dune::QuadratureRules<double, Traits::mydim>::rule(localView_.element().type(), order);
      Dune::StandardLocalFunction centerLineF(localBasisCenterLine, dispEleNodal,geoS);
      Dune::GeodesicLocalFunction quatF(localBasisQuaternions, quatEleNodal,geoS);
      using namespace Dune::DerivativeDirections;
      ScalarType energy = 0.0;
      for (const auto& gp : rule) {
        const auto a1            = toEigen(geo.jacobianTransposed(gp.position())).transpose().eval();
        const auto Jinv          = a1.norm();
        const auto r             = centerLineF.evaluateFunction(gp.position());
        const auto rGrad             = centerLineF.evaluateDerivative(gp.position(),Dune::wrt(spatialAll),Dune::on(gridElement));
        const auto q             = quatF.evaluate(gp.position());
        const auto qE = Eigen::Quaternion<ScalarType>(q);
        const auto qGrad             = quatF.evaluateDerivative(gp.position(),Dune::wrt(spatialAll),Dune::on(gridElement));
