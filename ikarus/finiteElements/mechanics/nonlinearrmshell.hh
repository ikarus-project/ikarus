...
namespace Ikarus {
  ...
  class NonLinearRMshell {
    ...
    struct KinematicVariables {
      ...
      Eigen::Matrix<field_type, 3, 2> a1anda2;  // first and second tangent base vector on the current geometry
      Eigen::Matrix<field_type, 3, 2>
      ...
      Eigen::Vector<field_type, 3> t;  // unit director on the current geometry
  ...
  public:
    ...
    NonLinearRMshell(const Basis &globalBasis, const typename LocalViewBlocked::Element &element,
                     const YoungsModulusAndPoissonsRatio &p_mat, const double thickness, const MultiTypeVector &p_x0,
      ...
      const double &Emodul = p_mat.emodul;  // Young's modulus
      const double &nu     = p_mat.nu;      // Poisson ratio
      CMat_.setZero();
      // membrane
    ...
    using Traits = TraitsFromLocalView<LocalViewFlat>;
    ...
    static Eigen::Vector<field_type, 8> calculateGreenLagrangianStrains(const KinematicVariables &kin) {
      Eigen::Vector<field_type, 8> egl;
      // membrane
    ...
    static Eigen::Vector<Scalar, 8> calculateStressResultants(const Eigen::Matrix<double, 8, 8> &Cred,
                                                              const Eigen::Vector<Scalar, 8> &Egl) {
      Eigen::Vector<Scalar, 8> S;
    ...
    Eigen::Matrix<field_type, 8, 3> boperatorMidSurface(const KinematicVariables &kin, int integrationPointIndex,
                                                        int coeffIndex, const auto &midSurfaceFunction) const {
      using namespace Dune::TypeTree::Indices;
      using namespace Dune::DerivativeDirections;
      const std::array<Eigen::Matrix<field_type, 3, 3>, 2> diffa1Anda2
      ...
      Eigen::Matrix<field_type, 8, 3> bop;
      bop.setZero();
    ...
    Eigen::Matrix<field_type, 8, 2> boperatorDirector(const KinematicVariables &kin, int integrationPointIndex,
                                                      int coeffIndex, const auto &directorFunction) const {
      using namespace Dune::TypeTree::Indices;
      using namespace Dune::DerivativeDirections;
      const std::array<Eigen::Matrix<field_type, 3, 2>, 2> diffdt
          = directorFunction.evaluateDerivative(integrationPointIndex, Dune::wrt(spatialAll, coeff(_1, coeffIndex)));
      const Eigen::Matrix<field_type, 3, 2> difft
      ...
      Eigen::Matrix<field_type, 8, 2> bop;
      bop.setZero();
    ...
    auto createFunctions(const FERequirementType &par) const {
      using namespace Dune::Indices;
      ...
      for (auto i = 0U; i < fe1.size(); ++i) {
        const auto globalIndex        = localViewBlocked_.index(localViewBlocked_.tree().child(_1, 0).localIndex(i));
        localDirectorConfiguration[i] = dNodal[globalIndex[1]];
      }

      const auto [localRefMidSurfaceConfiguration, localRefDirectorConfiguration] = getReferenceLocalConfigurations();

      std::vector<Dune::RealTuple<field_type, 3>> displacements;
      displacements.reserve(localMidSurfaceConfiguration.size());
      for (size_t i = 0; i < localMidSurfaceConfiguration.size(); ++i)
        displacements.emplace_back(localMidSurfaceConfiguration[i].getValue()
                                   - localRefMidSurfaceConfiguration[i].getValue());

      using namespace Dune::DerivativeDirections;
      Dune::StandardLocalFunction midSurfaceFunction(localBasisMidSurface, localMidSurfaceConfiguration, geo_, _0);
      Dune::StandardLocalFunction midSurfaceReferenceFunction(localBasisMidSurface, localRefMidSurfaceConfiguration,
                                                              geo_, _0);
      ...
      return std::make_tuple(midSurfaceFunction, midSurfaceReferenceFunction, displacementFunction, directorFunction,
    ...
    [[nodiscard]] typename Traits::ScalarType calculateScalar(const FERequirementType &par) const {
      Eigen::VectorXd dx(localViewFlat_.size());
      dx.setZero();
