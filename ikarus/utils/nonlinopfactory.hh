  ...
  static auto op(Assembler&& as, typename traits::remove_pointer_t<std::remove_cvref_t<Assembler>>::FERequirement& req,
    ...
    if constexpr (provideScalar) {
      ...
      if constexpr (provideVector and provideMatrix)
        ...
        return NonLinearOperator(functions(std::move(energyFunction), std::move(residualFunction)),
                                 parameter(req.globalSolution(), req.parameter()));
      else if constexpr (provideMatrix)
        return NonLinearOperator(functions(std::move(energyFunction), std::move(KFunction)),
                                 parameter(req.globalSolution(), req.parameter()));
      else
        return NonLinearOperator(functions(std::move(energyFunction)),
                                 parameter(req.globalSolution(), req.parameter()));
      ...
      if constexpr (provideMatrix)
        ...
        return NonLinearOperator(functions(std::move(residualFunction)),
                                 parameter(req.globalSolution(), req.parameter()));
    } else if constexpr (provideMatrix)
      return NonLinearOperator(functions(std::move(KFunction)), parameter(req.globalSolution(), req.parameter()));
    else
