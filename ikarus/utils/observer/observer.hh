...
public:
   ...
   */
  void subscribe(MessageType message, std::shared_ptr<IObserver<MessageType, InfoType>> observer);
  /**
   * \brief Subscribe an observer to receive notifications for all message types.
   * \param observer The observer to be subscribed.
   */
  void subscribeAll(std::shared_ptr<IObserver<MessageType, InfoType>> observer);
  /**
   ...
   */
  void subscribeAll(std::initializer_list<std::shared_ptr<IObserver<MessageType, InfoType>>> observers);
  /**
   ...
   */
  void unSubscribe(MessageType message, std::shared_ptr<IObserver<MessageType, InfoType>> observer);
  /**
   * \brief Unsubscribe an observer from receiving notifications for all message types.
   * \param observer The observer to be unsubscribed.
   */
  void unSubscribeAll(std::shared_ptr<IObserver<MessageType, InfoType>> observer);
  /**
...
private:
  using ObserverVector = std::vector<std::shared_ptr<IObserver<MessageType, InfoType>>>;
...
template <typename MT, typename IT>
void IObservable<MT, IT>::subscribe(MT message, std::shared_ptr<IObserver<MT, IT>> observer) {
  observers_[message];
...
template <typename MT, typename IT>
void IObservable<MT, IT>::subscribeAll(std::shared_ptr<IObserver<MT, IT>> observer) {
  for (auto& msg : messages_)
    subscribe(msg, observer);
}

template <typename MT, typename IT>
void IObservable<MT, IT>::subscribeAll(std::initializer_list<std::shared_ptr<IObserver<MT, IT>>> observers) {
...
template <typename MT, typename IT>
void IObservable<MT, IT>::unSubscribe(MT message, std::shared_ptr<IObserver<MT, IT>> observer) {
  auto vectorOfObserversOfASpecificMessage = observers_[message];
  std::ranges::remove_if(vectorOfObserversOfASpecificMessage, [&observer](auto&& obs) { return obs == observer; });
}

template <typename MT, typename IT>
void IObservable<MT, IT>::unSubscribeAll(std::shared_ptr<IObserver<MT, IT>> observer) {
  for (auto& msg : messages_)
