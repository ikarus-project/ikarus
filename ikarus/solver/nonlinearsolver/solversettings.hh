...
#define MEMBERVARIABLESSETTER(name, type, default_value, comment)                                                  \
  {#name, [this](const pybind11::handle& value) {                                                                  \
     try {                                                                                                         \
       ...
       pybind11::cast_error eAmmend(("The value type of \"" + std::string(#name) + "\" should be something as <" + \
                                     std::string(#type) +                                                          \
                                     "> "                                                                          \
...
#define SOLVERSETTINGS(Name, FIELDS)                                                                         \
  ...
  {                                                                                                          \
    ...
    void populate(const pybind11::dict& dict) {                                                              \
      static const std::unordered_map<std::string, std::function<void(const pybind11::handle&)>> setters = { \
          FIELDS(MEMBERVARIABLESSETTER)};                                                                    \
                                                                                                             \
      for (auto item : dict) {                                                                               \
