...
template <typename ElementType, typename UserFunction = Impl::DefaultUserFunction>
class ResultFunction3D : public Dune::VTKFunctionMod<typename ElementType::GridView> {
 public:
  using ResultRequirements     = typename ElementType::ResultRequirementsType;
  using GridView               = typename ElementType::GridView;
  using ctype                  = typename GridView::ctype;
  constexpr static int griddim3D = 3;
  typedef typename GridView::template Codim<0>::Entity Entity;

  double evaluate(int comp, const Entity& e, const Dune::FieldVector<ctype, griddim3D>& local) const override {
    auto index = gridView.indexSet().index(e);
    return evaluateStressComponent(index, local, comp);
  }
  ...
  [[nodiscard]] constexpr int ncomps() const override {
    if constexpr (std::is_same_v<UserFunction, Impl::DefaultUserFunction>) {
      Dune::FieldVector<ctype, griddim3D> val(0.0);
 ...
 private:
  double evaluateStressComponent(int eleID, const Dune::FieldVector<ctype, griddim3D>& local, int comp) const {
    if constexpr (!std::is_same_v<UserFunction, Impl::DefaultUserFunction>)
      return userFunction_(fes_->at(eleID), resultRequirements_, local, comp);
    else {
      fes_->at(eleID).calculateAt(resultRequirements_, local, resultTypeMap);
