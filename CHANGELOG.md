...
- Big refactor of fe requirements and non linear operator construction ([#289](https://github.com/ikarus-project/ikarus/pull/289))
    - The fe requirement type is not any longer standalone but it is dictated by the element. Therefore, the elements are no longer a template
    ...
    - The assemblers now only implement a single function `matrix` and `vector`, where also affordance and the `EnforcingDBCOption` have to
    ...
    - The affordances now also now about each others relation therefore calling the function `vectorAffordance(MatrixAffordance::stiffness)`
