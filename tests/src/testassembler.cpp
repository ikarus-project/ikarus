...
auto SimpleAssemblersTest(const PreBasis& preBasis) {
  ...
  for (int ref = 0; ref < 4; ++ref) {
    ...
    if (not(Ikarus::Concepts::LagrangeNodeOfOrder<ChildType, 1> and ref == 0)) {
      ...
      sparseFlatAssembler.bind(doubleEnergy);
      sparseFlatAssembler.bind(pointLoad);
      sparseFlatAssembler.bind(addSpringStiffnessSparse);
      ...
      auto& mKRawDense = denseFlatAssembler.matrix(req, MatrixAffordance::stiffness, DBCOption::Raw);
      auto& mKRaw      = sparseFlatAssembler.matrix(req, MatrixAffordance::stiffness, DBCOption::Raw);
      auto& mRRawDense = denseFlatAssembler.vector(req, VectorAffordance::forces, DBCOption::Raw);
      auto& mRRaw      = sparseFlatAssembler.vector(req, VectorAffordance::forces, DBCOption::Raw);
      checkAssembledQuantities(t, KRaw, KRawDense, totalDOFs);
      checkAssembledQuantities(t, RRaw, RRawDense, totalDOFs);

      auto& mKDense = denseFlatAssembler.matrix(req, MatrixAffordance::stiffness, DBCOption::Full);
      auto& mK      = sparseFlatAssembler.matrix(req, MatrixAffordance::stiffness, DBCOption::Full);
      auto& mRDense = denseFlatAssembler.vector(req, VectorAffordance::forces, DBCOption::Full);
      auto& mR      = sparseFlatAssembler.vector(req, VectorAffordance::forces, DBCOption::Full);
      checkAssembledQuantities(t, K, KDense, totalDOFs);
      checkAssembledQuantities(t, R, RDense, totalDOFs);
      const double newEnergyDense = denseFlatAssembler.scalar(req, ScalarAffordance::mechanicalPotentialEnergy);
      const double newEnergy      = sparseFlatAssembler.scalar(req, ScalarAffordance::mechanicalPotentialEnergy);
      checkScalars(t, newEnergy, newEnergyDense, " Incorrect energies for sparse and dense assemblers");
      checkScalars(t, newEnergy, 2.0 * energy, " Incorrect energy for sparse assembler after manipulation");
      checkScalars(t, newEnergy, 4.0 * energyDense, " Incorrect energy for dense assembler after manipulation");
      for (int i = 0; i < 2; ++i) {
        checkScalars(t, KRawDense.diagonal()[i] + springStiffness, mKRawDense.diagonal()[i],
                     " Mismatch in KRawDense and mKRawDense at i = " + std::to_string(i));
        checkScalars(t, KRaw.diagonal()[i] + springStiffness, mKRaw.diagonal()[i],
                     " Mismatch in KRaw and mKRaw at i = " + std::to_string(i));
        checkScalars(t, KDense.diagonal()[i] + springStiffness, mKDense.diagonal()[i],
                     " Mismatch in KDense and mKDense at i = " + std::to_string(i));
        checkScalars(t, K.diagonal()[i] + springStiffness, mK.diagonal()[i],
                     " Mismatch in K and mK at i = " + std::to_string(i));
        checkScalars(t, RRawDense[i] + pointF, mRRawDense[i],
                     " Mismatch in RRawDense and mRRawDense at i = " + std::to_string(i));
        checkScalars(t, RRaw[i] + pointF, mRRaw[i], " Mismatch in RRaw and mRRaw at i = " + std::to_string(i));
        checkScalars(t, RDense[i] + pointF, mRDense[i], " Mismatch in RDense and mRDense at i = " + std::to_string(i));
        checkScalars(t, R[i] + pointF, mR[i], " Mismatch in R and mR at i = " + std::to_string(i));
      }
    }
