...
auto NonLinearElasticityLoadControlNRandTRforKLShell() {
  ...
  const std::vector<std::vector<ControlPoint>> controlPoints
      = {{{.p = {0, 0, 0}, .w = 1}, {.p = {0, 5, 0}, .w = 1}}, {{.p = {0, 0, 10}, .w = 1}, {.p = {0, 5, 10}, .w = 1}}};
  ...
  std::string lambdaNeumannVertices = std::string("lambda x: ( x[2]>9.999 )");
  Python::start();
  ...
  dirichletValues.fixDOFs([&](auto& basis, auto&& dirichletFlags) {
    for (int fixedDirection = 0; fixedDirection < 2; ++fixedDirection) {
      Dune::Functions::forEachBoundaryDOF(
          Dune::Functions::subspaceBasis(basis, fixedDirection),
          [&](auto&& localIndex, auto&& localView, auto&&) { dirichletFlags[localView.index(localIndex)] = true; });
    }
  });
  ...
  dirichletValues.fixDOFs([&](auto& basis, auto&& dirichletFlags) {
    Dune::Functions::forEachBoundaryDOF(Dune::Functions::subspaceBasis(basis, 2),
                                        [&](auto&& localIndex, auto&& localView, auto&& interSection) {
                                          if (interSection.geometry().center()[2] < 1e-8)
  ...
  const auto& K = KFunction(d, lambda);
  ...
  alc.subscribeAll({vtkWriter, pathFollowingObserver});
