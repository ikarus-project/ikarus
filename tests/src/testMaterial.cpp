...
auto testMaterial(Material mat) {
  ...
  if constexpr (
      std::is_same_v<
          Material,
          LinearElasticity<
              double>> or std::is_same_v<Material, Ikarus::VanishingStress<std::array<Ikarus::Impl::StressIndexPair, 1>({{2, 2}}), Ikarus::LinearElasticity<double>>>) {
    t.subTest(testMaterialWithStrain<StrainTags::linear>(mat));
                                      ...
                                      Ikarus::VanishingStress<std::array<Ikarus::Impl::StressIndexPair, 1>({{2, 2}}),
                                                              Ikarus::StVenantKirchhoff<double>>>) {
