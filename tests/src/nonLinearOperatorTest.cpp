...
TEST(NonLinearOperator, SecondOrderVectorValuedOperator) {
  ...
  EXPECT_TRUE(checkGradient(nonLinOp, {.draw = false, .writeSlopeStatementIfFailed = false}));
...
TEST(NonLinearOperator, SecondOrderVectorValuedOperatorNonlinearAutodiff) {
  ...
  EXPECT_TRUE(checkGradient(nonLinOp, {.draw = false, .writeSlopeStatementIfFailed = false}));
  EXPECT_TRUE(checkHessian(nonLinOp, {.draw = false, .writeSlopeStatementIfFailed = false}));
