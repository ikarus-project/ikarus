...
MATCHER_P2(EigenApproxEqual, expect, prec,
  ...
  if constexpr (requires { arg.isApprox(expect, prec);  ( arg-expect).isMuchSmallerThan(1,prec); })
    return arg.isApprox(expect, prec) or ( arg-expect).isMuchSmallerThan(1,prec);
  else  if constexpr (requires { arg.isApprox(expect, prec);})
    return arg.isApprox(expect, prec) ;
  else  // Eigen::DiagonalMatrix branch
    return arg.diagonal().isApprox(expect.diagonal(), prec) or  (arg.diagonal()-expect.diagonal()).isMuchSmallerThan( 1,prec);
}
