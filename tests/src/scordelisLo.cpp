...
auto NonLinearElasticityLoadControlNRandTRforRMShell(char** argv) {
  ...
  const std::vector<std::vector<ControlPoint>> controlPoints
      = {{{.p = {-16.06969, 0, 19.15111 }, .w = 1}, {.p = {-16.06969, 50, 19.15111}, .w = 1}},
         {{.p = {0, 0,  32.6352 }, .w = 0.766044}, {.p = {0, 50, 32.6352 }, .w =  0.766044}},
         {{.p = {16.06969 , 0,  19.15111  }, .w = 1}, {.p = {16.06969 , 50,  19.15111 }, .w =  1}}
  };
...
//  BoundaryPatch<decltype(gridView)> neumannBoundary(gridView, neumannVertices);
  ...
  dirichletValues.fixBoundaryDOFs([&](auto& dirichletFlags, auto&& localIndex, auto&& localView, auto&& intersection) {
    if (std::abs(intersection.geometry().center()[1]) < 1e-8 or std::abs(intersection.geometry().center()[1]) > 50-1e-8) dirichletFlags[localView.index(localIndex)] = true;
  });
