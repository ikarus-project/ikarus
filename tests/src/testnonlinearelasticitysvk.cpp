...
int main(int argc, char** argv) {
  ...
  };
  {
    ...
    t.subTest(checkFEByAutoDiff<NonLinearElastic>(gridView,power<2>(lagrange<1>()),reducedMat,volumeLoad,&neumannBoundary,neumannBoundaryLoad));
  }

  {
    ...
    BoundaryPatch neumannBoundary(gridView, neumannVertices);
    t.subTest(checkFEByAutoDiff<NonLinearElastic>(gridView,power<3>(lagrange<1>()),matSVK1,volumeLoad,&neumannBoundary,neumannBoundaryLoad));
  }
