...
# https://github.com/dune-project/dune-common/blob/master/cmake/modules/FindMETIS.cmake
...
set(METIS_API_VERSION
    0
    CACHE STRING "METIS API version provided by METIS or scotch-metis library"
)
...
find_file(
  METIS_HEADER_FILE metis.h
  PATHS ${METIS_INCLUDE_DIR}
  NO_DEFAULT_PATH
)
if(METIS_HEADER_FILE)
  file(READ "${METIS_HEADER_FILE}" metisheader)
  string(REGEX REPLACE ".*#define METIS_VER_MAJOR[ ]+([0-9]+).*" "\\1" METIS_MAJOR_VERSION
                       "${metisheader}"
  )
  string(REGEX REPLACE ".*#define METIS_VER_MINOR[ ]+([0-9]+).*" "\\1" METIS_MINOR_VERSION
                       "${metisheader}"
  )
...
# We need to check whether we need to link m, copy the lazy solution from FindBLAS and FindLAPACK
# here.
...
# If scotch is requested, find package PTScotch and check version compatibility: Scotch provides
# METIS-3 interface only in version < 6.07, but provides an option to select the API-version in
# later Scotch releases
if(IS_SCOTCH_METIS_HEADER)
  ...
  if(PTScotch_SCOTCH_FOUND AND NOT METIS_API_VERSION)
    if(PTScotch_VERSION VERSION_LESS "6.0.7")
      ...
      check_symbol_exists(
        "METIS_PartGraphVKway" "stdio.h;stdint.h;scotch.h;metis.h" IS_SCOTCH_METIS_API_V3
      )
      if(IS_SCOTCH_METIS_API_V3)
        ...
        check_symbol_exists(
          "METIS_PartGraphKway" "stdio.h;stdint.h;scotch.h;metis.h" IS_SCOTCH_METIS_API_V5
        )
...
find_package_handle_standard_args(
  "METIS"
  REQUIRED_VARS METIS_LIBRARY METIS_INCLUDE_DIR METIS_API_VERSION
...
if(METIS_FOUND AND NOT TARGET METIS::METIS)
  ...
  if(IS_SCOTCH_METIS_HEADER AND PTScotch_FOUND)
    set_property(
      TARGET METIS::METIS
      APPEND
      PROPERTY INTERFACE_LINK_LIBRARIES PTScotch::Scotch
    )
    set_property(
      TARGET METIS::METIS
      APPEND
      PROPERTY INTERFACE_COMPILE_DEFINITIONS SCOTCH_METIS_VERSION=${METIS_API_VERSION}
    )
  endif()
endif()
