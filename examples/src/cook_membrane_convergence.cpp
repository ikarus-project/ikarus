  ...
  for(size_t nep=0; nep < easSet.size(); ++nep) {
      ...
      for (size_t ref = 0; ref < 9; ++ref) {
        auto start = std::chrono::high_resolution_clock::now();
          ...
          auto nonLinOp
                  = Ikarus::NonLinearOperator(linearAlgebraFunctions(residualFunction, KFunction),
          ...
          spdlog::info("The assembly took {:>6d} milliseconds with {} EAS parameters and {:>7d} dofs", durationAssembly.count(),numberOfEASParameters,basis.size());;
...
//          spdlog::info("The solver took {} milliseconds with {} EAS parameters and {} refinement level", durationSolver.count(),numberOfEASParameters,ref);
        ...
        vtkWriter.write("Cook_MembraneConvergence"+std::to_string(ref));
          ...
          spdlog::info("The total execution took {:>6d} milliseconds with {} EAS parameters and {:>7d} dofs", duration.count(),numberOfEASParameters,basis.size());
          grid->globalRefine(1);
      }
