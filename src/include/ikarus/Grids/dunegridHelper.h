...
namespace Dune {
  ...
  auto volumes(GV& gv) { return entities(gv, Dune::Dim<3>(), Dune::Partitions::all); }

  template <typename GV>
  requires requires(GV gv) { entities(gv, Dune::Dim<3>(), Dune::Partitions::all); }
  auto volumes(GV const& gv) { return entities(gv, Dune::Dim<3>(), Dune::Partitions::all); }

  template <typename GV>
  requires requires(GV gv) { entities(gv, Dune::Dim<2>(), Dune::Partitions::all); }
  auto surfaces(GV& gv) { return entities(gv, Dune::Dim<2>(), Dune::Partitions::all); }

  template <typename GV>
  requires requires(GV gv) { entities(gv, Dune::Dim<2>(), Dune::Partitions::all); }
  auto surfaces(GV const& gv) { return entities(gv, Dune::Dim<2>(), Dune::Partitions::all); }

  template <typename GV>
  requires requires(GV gv) { entities(gv, Dune::Dim<1>(), Dune::Partitions::all); }
  auto edges(GV& gv) { return entities(gv, Dune::Dim<1>(), Dune::Partitions::all); }

  template <typename GV>
  requires requires(GV gv) { entities(gv, Dune::Dim<1>(), Dune::Partitions::all); }
  auto edges(GV const& gv) { return entities(gv, Dune::Dim<1>(), Dune::Partitions::all); }
