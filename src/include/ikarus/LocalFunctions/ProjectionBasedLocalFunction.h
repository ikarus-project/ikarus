...
namespace Ikarus {
  ...
  class ProjectionBasedLocalFunction : public LocalFunctionInterface<ProjectionBasedLocalFunction<DuneBasis,CoeffContainer>> {
...
//    /** \brief Dimension of the coeffs */
  ...
  private:
    ...
    auto evaluateDerivativeWRTCoeffsANDSpatialImpl(const long unsigned gpIndex, const Manifold& val,const TransformMatrix& transformJ,
                                                   int coeffsIndex) const {
      const AnsatzFunctionJacobian dN = (basis.getJacobian(gpIndex)* transformJ);
      const GlobalE valE = evaluateEmbeddingFunctionImpl(basis.getFunction(gpIndex));
