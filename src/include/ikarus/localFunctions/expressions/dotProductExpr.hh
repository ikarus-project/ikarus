...
//            std::cout << Dune::className(u) << std::endl;
//            std::cout << Dune::className(v) << std::endl;
            std::array<std::remove_cvref_t<decltype( Ikarus::eval(
                transpose(v)*u_xy[0]))>, gridDim> res;
            for (int i = 0; i < gridDim; ++i)
              res[i] = Ikarus::eval(
                  transpose(v)*u_xy[i] + transpose(u_x.col(i))*v_y + transpose(v_x.col(i))*u_y + transpose(u)*v_xy[i]);
...
//          return Ikarus::eval(transpose(v) * u_xy + transpose(u_x) * v_y + transpose(v_x) * u_y + transpose(u) * v_xy);
        ...
        if constexpr (LFArgs::hasOneSpatialSingle) {
          ...
          static_assert(uTimesA.RowsAtCompileTime ==E1Raw::valueSize);
          static_assert(vTimesA.RowsAtCompileTime ==E2Raw::valueSize);
          ...
          static_assert(graduTimesA.RowsAtCompileTime == E1Raw::valueSize);
