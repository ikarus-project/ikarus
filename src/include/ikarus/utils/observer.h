...
public:
  ...
  void update(MessageType message) { assert(MessageType::END !=message && "The END enum type should not be used"); updateImpl(message);};
  void update(MessageType message, double val) { assert(MessageType::END !=message && "The END enum type should not be used"); updateImpl(message,val);};
