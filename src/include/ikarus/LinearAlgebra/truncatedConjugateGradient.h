...
namespace Eigen {
  ...
  * \param tol_error On input the tolerance error, on output an estimation of the relative error.
    ...
    template<typename MatrixType, typename Rhs, typename Dest, typename Preconditioner>
        void truncated_conjugate_gradient(const MatrixType& mat, const Rhs& rhs, Dest& x,
                                     const Preconditioner& precond, Eigen::Index& iters,
                                     typename Dest::RealScalar& tol_error, TCGInfo<typename Dest::RealScalar> &_info)
    {
      using std::sqrt;
      using std::abs;
      ...
      while(i < maxIters)
      {
        ...
        if (i >= _info.mininner && residualNorm2 <= rhsNorm2*std::min(rhsNorm2, _info.kappa)) //missing pow(rhsNorm2,_info.theta
        {
...
/** \ingroup IterativeLinearSolvers_Module
    ...
    */
  template< typename _MatrixType, int _UpLo, typename _Preconditioner>
  class TruncatedConjugateGradient : public IterativeSolverBase<TruncatedConjugateGradient<_MatrixType,_UpLo,_Preconditioner> >
  {

  public:
    typedef IterativeSolverBase<TruncatedConjugateGradient> Base;
    TruncatedConjugateGradient(TruncatedConjugateGradient&&other) noexcept : Base(std::move(other)), algInfo{other.algInfo} { };
  private:
    using Base::matrix;
    using Base::m_error;
  ...
  public:
    ...
    TCGInfo<typename _MatrixType::RealScalar> getInfo()
    {
      return algInfo;
    }


    void setInfo(TCGInfo<typename _MatrixType::RealScalar> _alginfo)
    {
      this->algInfo= _alginfo;
    }
    /** Default constructor. */
    ...
    * matrix A, or modify a copy of A.
    ...
    void _solve_vector_with_guess_impl(const Rhs& b, Dest& x) const
    {
      ...
      internal::truncated_conjugate_gradient(SelfAdjointWrapper(row_mat), b, x, Base::m_preconditioner, m_iterations, m_error, algInfo);
