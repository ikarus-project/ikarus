...
namespace Ikarus::FiniteElements {
  ...
  public:
    ...
    ScalarType calculateScalarImpl(const typename Traits::FERequirementType &req,
    ...
    auto calculateStiffnessMatrixAndInternalForcesImpl(
      ...
      if constexpr (internalForcesFlag && stiffnessMatrixFlag) {
        ...
        //        typename Traits::VectorType Fint(this->dofSize());
        //        Fint.setZero();
        //        const auto rule = Dune::QuadratureRules<double,
        //        Traits::mydim>::rule(duneType(elementGridEntity->type()), 2); Eigen::Matrix3d C; C.setZero(); const
        //        double fac = emod_ / (1 - nu_ * nu_); C(0, 0) = C(1, 1) = 1; C(0, 1) = C(1, 0) = nu_; C(2, 2) = (1 -
        //        nu_) / 2; C *= fac; for (auto &gp : rule) {
        //          const auto geo = elementGridEntity->geometry();
        //          const auto J   = toEigenMatrix(geo.jacobianTransposed(gp.position()));
        ...
        //              = Ikarus::LagrangeCube<double, Traits::mydim,
        //              1>::evaluateJacobian(toEigenVector(gp.position()));
        //          const auto Jloc = Ikarus::LinearAlgebra::orthonormalizeMatrixColumns(J.transpose());
        //          //          dN *= (J * Jloc).inverse();
        //          const auto j                                                = deformedgeo.jacobianTransposed(dN, x);
        //          const Eigen::Matrix<double, Traits::mydim, Traits::mydim> F = Jloc.transpose() * j * (J.inverse()) *
        //          Jloc;
        //                    dN *= (J * Jloc).inverse();
        //          const auto bop = boperator(dN, F);
        ////          std::cout<<"FinFint:\n"<<F<<std::endl;
        //          const auto E
        //              = (0.5 * (F.transpose() * F - Eigen::Matrix<double, Traits::mydim,
        //              Traits::mydim>::Identity())).eval();
