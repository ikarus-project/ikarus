...
namespace Ikarus {
  ...
  inline std::string print(GeometryType type) {
    switch (type) {
      ...
      case GeometryType::prism:
        return "prism";
      case GeometryType::linearHexahedron:
        return "linearHexahedron";
      default:
        throw std::runtime_error(
            "ostream operator for this GeometryType is not implemented. Check implementation in GeometryType.h");
    }
  }
  ...
  inline Dune::GeometryType duneType(Ikarus::GeometryType ikarusType) {
    switch (ikarusType) {
      ...
      default:
        throw std::runtime_error("The type " + print(ikarusType) + " has no equivalent Dune::GeometryType.");
    }
  }
