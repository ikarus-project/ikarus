...
void update( GenericVariable & vo , const Eigen::Ref<const Ikarus::DynVectord>& correction){   return vo.variableImpl->do_update(correction);}
void setValue( GenericVariable & vo , const Eigen::Ref<const Ikarus::DynVectord>& value){   return vo.variableImpl->do_setValue(value);}
...
bool  operator==(const GenericVariable & var,const GenericVariable & other ){   return var.variableImpl->do_equalComparison(other);}
bool  operator<(const GenericVariable & var,const GenericVariable & other ){   return var.variableImpl->do_lessComparison(other);}
...
{
   return std::accumulate(varSpan.begin(), varSpan.end(), 0, [](size_t cursize, const GenericVariable& var ){ return cursize+correctionSize(var);});
}
...
{
    return std::accumulate(varSpan.begin(), varSpan.end(), 0, [](size_t cursize, const GenericVariable& var ){ return cursize+valueSize(var);});
}
