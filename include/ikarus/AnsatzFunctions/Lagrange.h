 ...
 * and modified using Eigen Types
    ...
    The ordering of the nodal shape functions are coordinate-wise and not counterclockwise or something similar!
            ...
            else // The general case
            {
                ...
                for (size_t i = 0; i < sizeOfShapeFunctions; i++) {
                    ...
                    for (unsigned int j = 0; j < dim; j++) {
                        ...
                        for (unsigned int l = 0; l < dim; l++)
                            if (l != j)
                                dN(i,j) *= getAnsatzFunctionImpl(alpha[l], x[l]);
                    }
                }
            }
            return dN/2;
...
// *
// * This implementation is taken from https://gitlab.dune-project.org/core/dune-localfunctions/-/blob/master/dune/localfunctions/lagrange/lagrangesimplex.hh
// * and modified using eigen types
//
//Lagrange shape functions of arbitrary order have the property that
//\f$\hat\phi^i(x_j) = \delta_{i,j}\f$ for certain points \f$x_j\f$.
...
//\tparam k Polynomial order
//*/
//template<class ScalarType, unsigned int dim, unsigned int k, std::enable_if<!(dim!=3 && k>=2),bool> = true>
//class LagrangeSimplex
//{
// public:
// public:
//  /** \brief Number of shape functions */
...
//            N[n] *= (lagrangeNode(gamma)-x[0]-x[1])/(lagrangeNode(gamma)-lagrangeNode(i)-lagrangeNode(j));
//          n++;
//        }
//
//      return;
