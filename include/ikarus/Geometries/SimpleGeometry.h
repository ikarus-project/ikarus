...
template<typename ct, int wdim, int geodim,typename ShapeFunctionType, std::enable_if_t<geodim<=wdim, bool> = true>
class SimpleGeometry {
 public:
   ...
   *
   * \note E.g. if the size of the shapefunctions are know at compile time we use a fixed size Matrix
   *   If we have a Q1 element in 3D space we have FixedMatrixd<3,4>
   *   If we have an element with unknown shape function we have FixedMatrixd<3,Dynamic>
   * */
  using VertexContainerType = typename std::conditional< ShapeFunctionType::sizeOfShapeFunctions == Ikarus::Dynamic,
                                                         FixedRowHybridMatrixd<mydimension>,
                                                         FixedMatrixd<mydimension,ShapeFunctionType::sizeOfShapeFunctions> >::type;


  SimpleGeometry(const VertexContainerType& verticesInput)
  ...
  JacobianTransposedType jacobianTransposed(const LocalCoordinate& x)
  {
    ...
    static_assert(JacobianTransposedType::RowsAtCompileTime==mydimension,"Mismatch between shape function derivatives and the space of the geometry.");
