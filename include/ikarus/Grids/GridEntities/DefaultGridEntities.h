 ...
 **/
    ...
    public:
        ...
        DefaultGridEntity(int levelInput,size_t idInput)
      :levelIndex{levelInput},
      id{idInput}
        {}

        auto& getChildVertices()
        {
            return std::get<0>(entitiesChildren);
        }

      const auto& getChildVertices() const
      {
        return std::get<0>(entitiesChildren);
      }
      ...
      using Geometry = Dune::MultiLinearGeometry<double ,codimension,dimensionworld>;
        ...
        using  EntitiesChildernType = decltype(  Impl::ChildEntityPointerTupleGenerator<dimension,mydimension,dimensionworld>(std::make_integer_sequence< int, mydimension >()));

      /** \brief Returns the number of subEntities of this entity, e.g. a line has two verteces as subtypes */
      [[nodiscard]] unsigned int subEntities( unsigned int codim) const
      {
        ...
        {
          if (codim==1)       return getChildEntities<1>().size();
          else if (codim==2)  return getChildEntities<0>().size();
        }
      ...
      /** \brief Return the fundamental geometric type of the entity */
        ...
        auto geometry () const {
          ...
          for (const auto& pos : std::ranges::transform_view(getChildVertices(),&DefaultGridEntity<griddim,griddim,wdim>::getPosition))
 ...
 **/
    ...
    public:
      ...
      id{idInput},
       ...
       auto& getFatherElements()
        {
            return std::get<0>(entitiesFathers);
        }

      template<int coDimsubEnt>
      auto& getFatherEntities()
      {
        static_assert(coDimsubEnt>= 0 && coDimsubEnt<=griddim-1,"You asked for a non-existing FatherEntity!"); //TODO Fatherentities
        ...
        using  EntitiesFatherType = decltype(  Impl::FatherEntityPointerTupleGenerator<dimension,mydimension,dimensionworld>(std::make_integer_sequence< int, codimension >()));
 ...
 **/
    ...
    private:
        ...
        /** \brief Childrens of the entity on the current grid , i.e. surfaces of a cube*/
        decltype(  Impl::ChildEntityPointerTupleGenerator<dimension,mydimension,dimensionworld>(std::make_integer_sequence< int, mydimension >())) entitiesChildren;
