    ...
    namespace Impl {
      ...
      static_assert(dim<= dimworld, "The dimension of the grid can not be smaller that the embedding space.");
        public:
            ...
            using GridEntityTuple = decltype(Impl::GridEntityTupleGenerator<dim,dimworld>(std::make_integer_sequence< int, dimension+1 >()));
...
//          using GridView = SimpleGridView<dimension,dimensionworld,SimpleGrid>;
          ...
          auto leafGridView()
          {
            return  SimpleGridView<dimension,dimensionworld,SimpleGrid>(*this,0);
          }

         private:
            auto& getElements(int level = 0)
            {
                return getSubEntities<0>(level);
            }
