 ...
 *  the corresponding edges and surface. The numbering of vertices, edges, surfaces correspond to \cite sander2020dune Fig 5.12, 5.13
 *
...
void SimpleGridFactory<GridType>::
insertElement(const Dune::GeometryType &type, const DynArrayXi &verticesIn) {
  ...
  if (type.isLine()) {
    ...
    DUNE_THROW(Dune::GridError, "You cannot insert a " << type
                                                       << " into a SimpleGrid<" << dimensionworld << ">!");
  }
    ...
    //set elements pointers of vertex
      ...
      for (auto&& vert : grid.getVertices())
      {
        ...
        auto  hasVertex =[&vert](auto& edge){
          return (std::ranges::find_if(edge.getChildVertices(),[&vert](auto& vertex){return vert.getID()==vertex->getID();} ) != end(edge.getChildVertices()));};
//        auto  hasVertex =[&vert](auto& edge){
//          return true;};
